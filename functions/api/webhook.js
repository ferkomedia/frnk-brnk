export async function onRequestPost(context) {
  ...
  try {
    ...
    if (payload.type === 'payment_intent.succeeded') {
      ...
      // Ignoruj platby, ktoré nie sú za knihu (napr. z ferkomedia.sk workshopy/weby)
      var desc = (paymentIntent.description || '').toLowerCase();
      var product = (metadata.product || '').toLowerCase();
      var isFrnkOrder = desc.includes('frnk') || desc.includes('kniha') || product.includes('frnk') || product.includes('kniha') || metadata.source === 'frnk-brnk';
      if (!isFrnkOrder && !metadata.quantity) {
        return new Response(JSON.stringify({ received: true, skipped: 'not a book order' }), {
      ...
      const deliveryType = metadata.delivery_type || 'pickup';
      const deliveryCountry = metadata.delivery_country || '';
      ...
      const addressZip = metadata.address_zip || '';
      const addressCity = metadata.address_city || '';

      const countryNames = {
        'sk':'Slovensko','cz':'Česko','pl':'Poľsko','hu':'Maďarsko','ro':'Rumunsko',
        'bg':'Bulharsko','hr':'Chorvátsko','si':'Slovinsko','ee':'Estónsko','lv':'Lotyšsko',
        'lt':'Litva','de':'Nemecko','at':'Rakúsko','gb':'Veľká Británia','ie':'Írsko',
        'nl':'Holandsko','be':'Belgicko','fr':'Francúzsko','it':'Taliansko','es':'Španielsko',
        'pt':'Portugalsko','ch':'Švajčiarsko','se':'Švédsko','dk':'Dánsko','fi':'Fínsko',
        'gr':'Grécko','lu':'Luxembursko'
      };
      const countryName = countryNames[deliveryCountry.toLowerCase()] || deliveryCountry;

      const deliveryInfo = deliveryType === 'address'
          ? addressStreet + ', ' + addressZip + ' ' + addressCity + ', ' + countryName
          : packetaPoint;
      ...
      const customerEmailHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Andika:wght@400;700&display=swap" rel="stylesheet"></head><body style="margin:0;padding:0;background-color:' + colors.cream + ';"><table width="100%" cellpadding="0" cellspacing="0" style="background-color:' + colors.cream + ';padding:40px 20px;"><tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="background-color:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 16px rgba(44,62,45,0.12);">' +
          ...
          (deliveryType === 'address'
                  ? '<tr><td style="padding:20px 40px;"><div style="background-color:' + colors.paper + ';border-radius:12px;padding:24px;border:1px solid ' + colors.border + ';"><h3 style="margin:0 0 16px;color:' + colors.text + ';font-family:Andika,Georgia,serif;font-size:18px;">Doručenie na adresu</h3><p style="margin:0 0 8px;font-family:Andika,Georgia,serif;font-size:15px;color:' + colors.text + ';"><strong>Adresa:</strong></p><p style="margin:0 0 16px;font-family:Andika,Georgia,serif;font-size:15px;color:' + colors.textLight + ';">' + deliveryInfo + '</p><p style="margin:0;font-family:Andika,Georgia,serif;font-size:14px;color:' + colors.textLight + ';">O odoslaní balíka vás budeme informovať emailom s číslom zásielky.</p></div></td></tr>'
      ...
      const notifyEmailHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Andika:wght@400;700&display=swap" rel="stylesheet"></head><body style="margin:0;padding:0;background-color:' + colors.cream + ';"><table width="100%" cellpadding="0" cellspacing="0" style="background-color:' + colors.cream + ';padding:40px 20px;"><tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="background-color:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 16px rgba(44,62,45,0.12);">' +
          ...
          (deliveryType === 'address'
                  ? '<tr><td style="padding:0 40px 20px;"><div style="background-color:' + colors.sun + ';border-radius:12px;padding:24px;"><h3 style="margin:0 0 16px;color:' + colors.text + ';font-family:Andika,Georgia,serif;font-size:16px;">📍 Doručenie na adresu</h3><p style="margin:0 0 8px;font-family:Andika,Georgia,serif;font-size:15px;color:' + colors.text + ';"><strong>Adresa:</strong></p><p style="margin:0;font-family:Andika,Georgia,serif;font-size:15px;color:' + colors.text + ';">' + deliveryInfo + '</p></div></td></tr>'
                  : '<tr><td style="padding:0 40px 20px;"><div style="background-color:' + colors.sun + ';border-radius:12px;padding:24px;"><h3 style="margin:0 0 16px;color:' + colors.text + ';font-family:Andika,Georgia,serif;font-size:16px;">📦 Packeta</h3><p style="margin:0 0 8px;font-family:Andika,Georgia,serif;font-size:15px;color:' + colors.text + ';"><strong>Výdajné miesto:</strong></p><p style="margin:0 0 8px;font-family:Andika,Georgia,serif;font-size:15px;color:' + colors.text + ';">' + deliveryInfo + '</p><p style="margin:0;font-family:Andika,Georgia,serif;font-size:14px;color:' + colors.text + ';">ID: ' + packetaId + '</p></div></td></tr>'
          ) +
